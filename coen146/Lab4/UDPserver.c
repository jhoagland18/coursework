...
int main (int argc, char *argv[])
{
	...
	while (1)
	{
		...
		printf("exiting title block. Expecting next packet of %i\n\n",(last_ack+1 %2));
		...
		while(nBytes = recvfrom(sock,&packet_rcv,sizeof(packet_rcv), 0, (struct sockaddr *)&serverStorage, &addr_size)) {
			...
			printf("recieved ack is %i; last ack is %i\n",packet_rcv.header.seq_ack, last_ack);
			//if corrupted
			...
			} else if(packet_rcv.header.seq_ack != (last_ack + 1) % 2) {//incorrect packet sequence	
				printf("recieved out of order ack # %i\n",packet_rcv.header.seq_ack);
				packet_rcv.header.seq_ack = (packet_rcv.header.seq_ack + 1) % 2;
			...
			} else { //if packet is good
				printf("sending %i\n",packet_rcv.header.seq_ack);
				if(packet_rcv.header.fin==1) { //if fin packet
					sendto (sock, &packet_rcv, sizeof(packet_rcv), 0, (struct sockaddr *)&serverStorage, addr_size);
					printf("EOF reached.\n");
					break;
